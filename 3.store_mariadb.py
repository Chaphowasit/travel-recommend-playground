        ...
        for key, value in entry.items():
            if key not in schema:
                ...
                if isinstance(value, str):
                    if len(value) > 255:
                        schema[key] = 'TEXT'
                elif isinstance(value, int) and schema[key] != 'INT':
                    schema[key] = 'TEXT'
                elif isinstance(value, float) and schema[key] != 'DOUBLE':
                    schema[key] = 'TEXT'
                elif isinstance(value, bool) and schema[key] != 'BOOLEAN':
                    schema[key] = 'TEXT'
    print(schema)
    return schema
...
def process_json_files(connection, json_directory, table_name):
    ...
    for json_file in json_files:
        ...
        print(f"Schema for '{input_file}': {schema}")
