...
def extract_nearby_places(soup, best_nearby_hotels):
    ...
    if nearby_places:
        ...
        try:
            index_best_nearby_hotels = safe_index(
                nearby_places_text, "Best nearby hotels"
            )
            index_best_nearby_restaurants = safe_index(
                nearby_places_text, "Best nearby restaurants"
            )
            index_best_nearby_attractions = safe_index(
                nearby_places_text, "Best nearby attractions"
            )

            classification_nearby["nearby_accommodation1"] = (
                nearby_places_text[index_best_nearby_hotels + 1]
                if index_best_nearby_hotels + 1 < index_best_nearby_restaurants
                else None
            )
            classification_nearby["nearby_accommodation2"] = (
                nearby_places_text[index_best_nearby_hotels + 2]
                if index_best_nearby_hotels + 2 < index_best_nearby_restaurants
                else None
            )
            classification_nearby["nearby_accommodation3"] = (
                nearby_places_text[index_best_nearby_hotels + 3]
                if index_best_nearby_hotels + 3 < index_best_nearby_restaurants
                else None
            )

            classification_nearby["nearby_foodAndDrink1"] = (
                nearby_places_text[index_best_nearby_restaurants + 1]
                if index_best_nearby_restaurants + 1 < index_best_nearby_attractions
                else None
            )
            classification_nearby["nearby_foodAndDrink2"] = (
                nearby_places_text[index_best_nearby_restaurants + 2]
                if index_best_nearby_restaurants + 2 < index_best_nearby_attractions
                else None
            )
            classification_nearby["nearby_foodAndDrink3"] = (
                nearby_places_text[index_best_nearby_restaurants + 3]
                if index_best_nearby_restaurants + 3 < index_best_nearby_attractions
                else None
            )

            classification_nearby["nearby_activity1"] = (
                nearby_places_text[index_best_nearby_attractions + 1]
                if index_best_nearby_attractions + 1 < len(nearby_places_text)
                else None
            )
            classification_nearby["nearby_activity2"] = (
                nearby_places_text[index_best_nearby_attractions + 2]
                if index_best_nearby_attractions + 2 < len(nearby_places_text)
                else None
            )
            classification_nearby["nearby_activity3"] = (
                nearby_places_text[index_best_nearby_attractions + 3]
                if index_best_nearby_attractions + 3 < len(nearby_places_text)
                else None
            )
        except ValueError:
            pass
    ...
    if not any(
        [classification_nearby[f"nearby_foodAndDrink{i}"] for i in range(1, 4)]
    ) or not any([classification_nearby[f"nearby_activity{i}"] for i in range(1, 4)]):
        nearby_places = soup.find_all(class_="xCVkR")
        if nearby_places:
            res = [
                place for place in nearby_places if "Restaurants" in place.text.strip()
            ]
            attr = [
                place for place in nearby_places if "Attractions" in place.text.strip()
            ]

            if res:
                res = [place.find_all(class_="o W q") for place in res]
                restaurants = [
                    item.text.strip() for sublist in res for item in sublist if item
                ][:3]
            ...
            if attr:
                ...
                attractions = [
                    item.text.strip() for sublist in attr for item in sublist if item
                ][:3]
    ...
    try:
        ...
        if not any(
            ...
            [classification_nearby[f"nearby_activity{i}"] for i in range(1, 4)]
        ):
            nearby_places = soup.find_all(class_="yvHvW")
            if nearby_places[0] and nearby_places[1]:
                res = nearby_places[0].find_all(
                    class_="biGQs _P alXOW oCpZu GzNcM nvOhm UTQMg ZTpaU ngXxk"
                )
                attr = nearby_places[1].find_all(
                    class_="biGQs _P alXOW oCpZu GzNcM nvOhm UTQMg ZTpaU ngXxk"
                )

                restaurants = [
                    remove_newline_and_extra_spaces(restaurant.text.strip())
                    for restaurant in res
                    if restaurant.text.strip() != ""
                ][:3]
                attractions = [
                    remove_newline_and_extra_spaces(attraction.text.strip())
                    for attraction in attr
                    if attraction.text.strip() != ""
                ][:3]
...
def get_next_id(existing_ids, prefix):
    ...
    ids_with_prefix = [
        int(id[len(prefix) :]) for id in existing_ids if id.startswith(prefix)
    ]
    if ids_with_prefix:
...
def clean_html_and_classify(
    ...
    try:
        for entry in json_data:
            ...
            classification = {}
            existing_ids = [
                entry.get("classification", {}).get(f"{id_name}", "")
                for entry in json_data
                if "classification" in entry
            ]
            # Generate ID based on table name and existing IDs
            classification[id_name] = (
                generate_id(name_key, existing_ids) if name_key else None
            )
            ...
            if not paragraphs:
                ...
                paragraphs = [
                    ...
                    if "About" in para.text.strip()
                    and "Manage this business?" not in para.text.strip()
                ]
            if paragraphs:
                classification["about_and_tags"] = [
                    remove_newline_and_extra_spaces(paragraph.text.strip())
            ...
            if type(time_class) == tuple and len(time_class) == 2:
                classification["start_time"], classification["end_time"] = (
                    convert_time_string(time_class[0]),
                    convert_time_string(time_class[1]),
                )
            else:
                start_end_time = soup.find_all(class_=time_class)
                if start_end_time:
                    time_classes = [
                        remove_newline_and_extra_spaces(time)
                        for time in start_end_time[0].text.split("-")
                    ]
            ...
            reviews = soup.find_all(class_=reviews_class) or soup.find_all(
                class_="partial_entry"
            )
            ...
            if reviews_class == "dodo":
                ...
                reviews_inner = [
                    review.find_all(class_="yCeTE") for review in reviews_outer
                ]
                reviews_inner_flat = [
                    item for sublist in reviews_inner for item in sublist
                ]  # Flatten the list

                if reviews_inner_flat:
                    classification["reviews"] = [
                        review.text.strip() for review in reviews_inner_flat
                    ]
                    classification["reviews"] = [
                        remove_newline_and_extra_spaces(
                            re.sub(r"[^\x00-\x7F]+", "", review.text.strip())
        ...
        print(f"An error occurred while processing the data: {ex}")
...
def process_files(
    ...
    for json_file in json_files:
        ...
        output_file = os.path.join(
            output_directory, f"{os.path.splitext(json_file)[0]}_clean.json"
        )
        ...
        try:
            ...
            cleaned_data = clean_html_and_classify(
                json_data,
                name_key,
                desc_class,
                reviews_class,
...
# )
process_files(
    "do_add300/prettier_json",
    "do_add300/extract_json",
    "activity_name",
    "USjYi _d",
    "dodo",
    "EFKKt",
